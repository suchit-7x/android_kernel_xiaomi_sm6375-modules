...
/*
 * struct ipa_ncm_init_params - parameters for driver initialization API
 *
...
#if IS_ENABLED(CONFIG_NCM_IPA)

int ncm_ipa_init(struct ipa_ncm_init_params *params);

int ncm_ipa_pipe_connect_notify(u32 usb_to_ipa_hdl,
			u32 ipa_to_usb_hdl,
...
#else /* IS_ENABLED(CONFIG_NCM_IPA) */

static inline int ncm_ipa_init(struct ipa_ncm_init_params *params)
{
	return -ENOMEM;
}

static inline int ncm_ipa_pipe_connect_notify(u32 usb_to_ipa_hdl,
			u32 ipa_to_usb_hdl,
			u32 max_xfer_size_bytes_to_dev,
			u32 max_packet_number_to_dev,
			u32 max_xfer_size_bytes_to_host,
...
static inline int ncm_ipa_pipe_disconnect_notify(void *private)
{
	return -ENOMEM;
}

static inline void ncm_ipa_cleanup(void *private)
{
}
