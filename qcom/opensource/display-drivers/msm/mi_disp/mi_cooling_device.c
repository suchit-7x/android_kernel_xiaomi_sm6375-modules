					...
					unsigned long state)
{
	...
	if (mi_sde_cdev->panel->mi_cfg.thermal_dimming_enabled) {
		...
		DISP_TIME_INFO("thermal dimming:set thermal_brightness_limit to %lu percent\n",
				state);
	} else {
       brightness_lvl = mi_sde_cdev->panel->mi_cfg.max_brightness_clone * state / 200;
	   DISP_TIME_INFO("thermal dimming:set thermal_brightness_limit to %lu\n",
			   brightness_lvl);
			...
			struct device *dev, struct backlight_device *bd)
{
	...
	if (c_conn->connector_type != DRM_MODE_CONNECTOR_DSI) {
		DISP_ERROR("The connect type is not DSI\n");
