...
#define PMIC_PWRKEY_RESIN_BARK_TRIGGER 3
#define DISPLAY_DELAY_SHUTDOWN_TIME_MS 800
...
enum lhbm_update_status {
	LHBM_STATUS_OFF,
	LHBM_STATUS_WHITE_1000NIT,
	LHBM_STATUS_WHITE_110NIT,
	LHBM_STATUS_GREEN_500NIT,
	LHBM_STATUS_MAX,
};
...
struct mi_dsi_panel_cfg {
	...
	bool need_aod_exit_set_backlight;
	bool need_aod_enter_set_backlight;
	...
	bool lhbm_0size_on;
	bool lhbm_gxzw;
	enum lhbm_update_status lhbm_status;
	enum hbm_level hbm_level;
	u8 lhbm_rgb_param[24];
	u8 whitebuf_1000_gir_on[6];
	u8 whitebuf_1000_gir_off[6];
	u8 whitebuf_110_gir_on[6];
	u8 whitebuf_110_gir_off[6];
	u8 lhbm_1200nits_gamma[19];
	u8 lhbm_250nits_gamma[19];

	bool aod_to_normal_bl_delay;
	u32 aod_to_normal_bl;
...
int mi_dsi_panel_match_fps_setting(struct dsi_panel *panel, struct dsi_display_mode *adj_mode);
