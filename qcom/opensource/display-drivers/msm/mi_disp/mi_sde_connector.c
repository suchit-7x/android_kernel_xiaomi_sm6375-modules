...
static irqreturn_t mi_esd_err_irq_handle(int irq, void *data)
{
	...
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		mi_dsi_panel_state_count(display->panel, PANEL_ESD, 1);
	}
...
int mi_sde_connector_check_layer_flags(struct drm_connector *connector)
{
	...
	if (connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
		...
		display = (struct dsi_display *)c_conn->display;
		if (mi_get_panel_id_by_dsi_panel(display->panel) != P17_PANEL_PA) {
		...
		if (flags.notificationshade_changed) {
			...
			if (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) == P17_PANEL_PA) {
				if (!flags.notificationshade_present &&
					...
					mi_dsi_update_backlight_in_aod(display->panel, true);
