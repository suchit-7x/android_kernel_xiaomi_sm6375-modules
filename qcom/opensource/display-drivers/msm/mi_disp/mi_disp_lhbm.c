...
static int mi_disp_lhbm_fod_set_disp_param(struct disp_lhbm_fod *lhbm_fod, u32 lhbm_value)
{
	...
	DISP_INFO("target_brightness = %d, feature_val = %d\n", atomic_read(&lhbm_fod->target_brightness), ctl.feature_val);
...
static int mi_disp_lhbm_fod_thread_fn(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((mi_get_panel_id_by_dsi_panel(lhbm_fod->display->panel) == P17_PANEL_PA)
...
int mi_disp_set_local_hbm(int disp_id, int local_hbm_value)
{
	...
	mi_disp_feature_event_notify_by_type(disp_id,
			MI_DISP_EVENT_LOCAL_HBM_VALUE, sizeof(local_hbm_value), local_hbm_value);
	if (atomic_read(&lhbm_fod->target_brightness) == LHBM_TARGET_BRIGHTNESS_OFF_FINGER_UP &&
			local_hbm_value == LHBM_TARGET_BRIGHTNESS_OFF_AUTH_STOP) {
		goto exit;
	}
	...
	if (panel->power_mode == SDE_MODE_DPMS_ON) {
		if (mi_cfg->is_em_cycle_16_pulse ) {
			if(mi_cfg->lhbm_gxzw && !mi_cfg->lhbm_0size_on ){
				...
				DISP_INFO("gxzw apper,0 size lhbm on\n");
				...
				DISP_INFO("gxzw quit,0 size lhbm off\n");
			}
		} else if(!mi_cfg->is_em_cycle_16_pulse &&  mi_cfg->lhbm_0size_on ){
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_LOCAL_HBM_OFF_TO_NORMAL_EM_CYCLE_16PULSE);
			mi_cfg->lhbm_0size_on = false;
			DISP_INFO("cvc off,0 size lhbm off\n");
		}
	}
	return 0;
}
