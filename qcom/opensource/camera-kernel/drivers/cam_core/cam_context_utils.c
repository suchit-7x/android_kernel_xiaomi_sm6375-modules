...
static int cam_context_allocate_mem_hw_entries(struct cam_context *ctx)
{
	...
	if (!ctx->hw_update_entry) {
		CAM_ERR(CAM_CTXT, "%s[%d] no memory for hw_update_entry",ctx->dev_name, ctx->ctx_id);
		return -ENOMEM;
	}
	...
	if (!ctx->in_map_entries) {
		CAM_ERR(CAM_CTXT, "%s[%d] no memory for in_map_entries", ctx->dev_name, ctx->ctx_id);
		rc = -ENOMEM;
		goto free_mem;
	}
	...
	ctx->out_map_entries = kcalloc(ctx->req_size, sizeof(struct cam_hw_fence_map_entry *),
		GFP_KERNEL);
	if (!ctx->out_map_entries) {
		CAM_ERR(CAM_CTXT, "%s[%d] no memory for out_map_entries", ctx->dev_name, ctx->ctx_id);
		goto free_mem;
	}
